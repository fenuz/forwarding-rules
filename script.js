...
function add_forwarding_rule() {
	...
	$.getJSON(
		ajaxurl,
		{action:'add_rule', url: newurl, domain: ndomain, pattern: npattern, nonce: nonce },
		function(data){
...
function edit_forwarding_rule(id) {
	...
	$.getJSON(
		ajaxurl,
		{ action: "edit_rule_display", nonce: nonce, id: id },
		function(data){
...
function remove_forwarding_rule(id) {
	...
	$.getJSON(
		ajaxurl,
		{ action: "delete_rule", id: id, nonce: nonce },
		function(data){
			if (data.success == 1) {
...
function edit_rule_save(id) {
	...
	$.getJSON(
		ajaxurl,
		{action:'edit_rule_save', url: newurl, id: id, domain: newdomain, pattern: newpattern, nonce: nonce },
		function(data){
			if(data.status == 'success') {
